...
public class MTRandom extends Random {
	...
	public MTRandom(byte[] buf) {
		super(0L);
		System.out.println("bytebufconstructor");
		setSeed(buf);
	}
	...
	private final void setSeed(int seed) {
		...
		for (mti = 1; mti < N; mti++) {
			...
			//if(mti >620 ){System.out.println(mt[mti]);}
		}
	...
	public final synchronized void setSeed(int[] buf) {
		...
		int length = buf.length; //always 2
		if (length == 0) throw new IllegalArgumentException("Seed buffer may not be empty");
		// ---- Begin Mersenne Twister Algorithm ----
		int i = 1, j = 0, k = (N > length ? N : length);
		setSeed(MAGIC_SEED);
		for (; k > 0; k--) {
			//System.out.print(mt[i-1]);System.out.print(" ");System.out.print(i);System.out.print("\n");
			mt[i] = (mt[i] ^ ((mt[i-1] ^ (mt[i-1] >>> 30)) * MAGIC_FACTOR2)) + buf[j] + j;
			//System.out.println(mt[i]);
			//System.out.print((mt[i] ^ ((mt[i-1] ^ (mt[i-1] >>> 30)) * MAGIC_FACTOR2)));System.out.print(" "); System.out.print(buf[j] + j);System.out.print(" ");System.out.print(mt[i]);System.out.print("\n");
			i++; j++;
			if (i >= N) { 
				//System.out.println("i624");
	...
	protected final synchronized int next(int bits) {
		...
		if (mti >= N) {             // generate N words at one time
			...
			for (;kk < N-1; kk++) {
				...
				//System.out.println(mt[kk]);
			}
			y = (mt[N-1] & UPPER_MASK) | (mt[0] & LOWER_MASK);
			mt[N-1] = mt[M-1] ^ (y >>> 1) ^ MAGIC[y & 0x1];
			//System.out.println(mt[623]);
			mti = 0;
		}
